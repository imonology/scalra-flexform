	...
	function c_table(fields, value, record_id) {
		...
		for (var i in fields) {
			...
			if (fields[i].type === 'line') {
				...
				html += '<td  '+(fields[i].type === 'print'? 'class="print"' : '')+' style="'+(td_style?td_style[0]:'')+' ; vertical-align:middle;' + (typeof invisible !== 'undefined' && invisible.includes(fields[i].id) ? "display: none;" : "") + '" colspan="'+col_f+'">' + fields[i].name + (fields[i].must ? '' : '') +  ' </td>';
				// show field content
				html += '<td style="' + (td_style?td_style[1]:'') + (typeof invisible !== 'undefined' && invisible.includes(fields[i].id) ? "display: none;" : "") + '" colspan="'+col_v+'">';
				...
				switch (fields[i].type) {
					...
					case 'choice':
						if (write && !is_lock) {
							...
							if (typeof invisible !== 'undefined' && invisible.includes(fields[i].id)) {
								html += '<option value="">';
							}
